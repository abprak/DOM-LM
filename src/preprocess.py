...
with open(TAGS_PATH, "r") as f:
    TAGS_OF_INTEREST = [line.strip() for line in f.readlines()]
...
def tokenize_elements(elements: List[etree._Element]):
    # TODO: add caching
    return tokenizer([represent_node(el) for el in elements],
                     return_tensors="pt",
                     truncation=True,
                     padding=True)
...
def extract_element_info(
    ...
    attrs = {
        ...
        if v and k in attrs_to_extract
    }
...
def _tokens_len(elements, token_repr):
    if len(elements) == 0:
        return 0
    return sum(sum(token_repr[el]['tokenizer_res']['attention_mask'])
               for el in elements)
...
def _get_subtrees(pre_order, post_order, token_repr, m, s):
    ...
    while len(new) != 0:
        ...
        while total_len > m:
            if len(visited) != 0:
                el_root = visited[0]
                num_child = sum(child in new or child in visited
                                for child in el_root)
            else:
                # pop from new if there are no nodes in visited left
                num_child = 2
...
def _get_depth(node: etree._Element,
               max_parent: Optional[etree._Element] = None):
    d = 0
...
def get_features(element: etree._Element):
    ...
    # all_tokens = tokenizer(prev_nodes,
    #                        return_tensors="pt",
    #                        truncation=True,
    #                        padding=True)
    # total non-padding tokens
    # token_idx = all_tokens['attention_mask'].sum().item()

    element_repr = represent_node(element)
    tokenizer_res = tokenizer(element_repr,
                              truncation=True)
...
def get_tree_features(t: etree._Element,
                      token_repr: dict,
                      max_seq_length: int):
    elem_idxs = {}
    result = {
    ...
    for i, el in enumerate(t):
        ...
        attention_mask.append(
            token_repr[el]['tokenizer_res']['attention_mask']
            )
        ...
        if el_parent in elem_idxs:
            # Should it be different from node_id?
            parent_node_idx = elem_idxs[el_parent] + 1
    ...
    for i, (el_result, input_ids, attn_mask) in enumerate(zip(el_results,
                                                              input_ids,
                                                              attention_mask)):
...
def extract_token_for_nodes(dom: etree._Element) -> dict:
    '''
    Create dictionary that store el_reprs
    and tokenizer_res for each node in the DOM
    '''
    res = dict()
...
def get_tree_features_ae_task(t: List,
                              token_repr: dict,
                              node2label: dict,
                              max_seq_length: int):
    elem_idxs = {}
    ...
    for i, el in enumerate(t):
        ...
        labels.append(align_labels(token_repr[el]['tokenizer_res'],
                                   el,
                                   node2label,
                                   label2id))
    ...
    input_ids = truncate(input_ids, max_length)
    attention_mask = truncate(attention_mask, max_length)
    for i, (el_result, input_id, attn_mask, label) in enumerate(
                                                        zip(el_results,
                                                            input_ids,
                                                            attention_mask,
                                                            labels)):
...
def align_labels(tokenizer_res: Any,
                 el: etree._Element,
                 node2label: str,
                 label2id: dict):
    label_ids = []
    ...
    for word_idx in word_ids:
        if word_idx is None:
            ...
            label_ids.append(label2id[node2label[el]]
                             if el in node2label else 0)
    return label_ids


def assign_label_for_nodes(dom: etree._Element,
                           text2label: dict):
