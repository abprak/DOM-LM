...
def _numpy_collate_batch(examples,
    ...
    """
    Collate `examples` into a batch, using the information
    in `tokenizer` for padding if necessary.
    """
...
def _torch_collate_batch(examples,
                         ...
                         pad_to_multiple_of: Optional[int] = None):
    """
    Collate `examples` into a batch, using the information in
    `tokenizer` for padding if necessary.
    """
    import torch
    ...
    are_tensors_same_length = \
        all(x.size(0) == length_of_first for x in examples)
    if are_tensors_same_length and \
            (pad_to_multiple_of is None or
             length_of_first % pad_to_multiple_of == 0):
...
class DataCollatorForDOMNodeMask(DataCollatorForWholeWordMask):
    ...
    def _whole_node_mask(self, input_nodes: List[int], max_predictions=512):
        ...
        num_to_predict = min(
            max_predictions, max(1,
                                 int(round(
                                        len(input_nodes) * self.mlm_probability
                                        )
                                     )
                                 )
                            )
        ...
        for unique_node in unique_nodes:
            ...
            indexes = [i for i, n in enumerate(input_nodes)
                       if n == unique_node]
        ...
        mask_labels = [1 if i in masked_idxs else 0
